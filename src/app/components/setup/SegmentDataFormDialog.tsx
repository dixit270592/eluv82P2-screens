...
import { type SegmentDataRow } from '../../data/accountSetup';
import { P2P_BRAND } from '../../tokens/brand';
import { UI_FONT_STACK as F } from '../../tokens/typography';

type SegmentDataFormDialogProps = {
  open: boolean;
  mode: 'create' | 'edit';
  initial: SegmentDataRow | null;
  segmentTypes: string[];
  onOpenChange: (open: boolean) => void;
  onSave: (row: SegmentDataRow) => void;
};

const emptyDraft = (): Omit<SegmentDataRow, 'id'> => ({
  segmentType: '',
  segmentData: '',
  description: '',
});

export function SegmentDataFormDialog({
  open,
  mode,
  initial,
  segmentTypes,
  ...
  const handleSave = () => {
    if (!canSave) return;
    onSave({
      id: initial?.id ?? `sd-${crypto.randomUUID()}`,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
          <Field label="Select Account Segment" required>
            <Select
            ...
            >
              <SelectTrigger
                ...
                style={{ fontFamily: F }}
              >
                <SelectValue placeholder="Select Account Segment" />
              </SelectTrigger>
              <SelectContent>
