...
import { AddressFormDialog } from '../../components/setup/AddressFormDialog';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
type SortKey = 'name' | 'addressType' | 'city' | 'zipCode' | 'phone' | 'active';
type SortDir = 'asc' | 'desc';
type DeleteMode = { type: 'single'; id: string; name: string } | { type: 'bulk'; count: number };

export function AddressSetup() {
  const [items, setItems] = useState<AddressRecord[]>(() => createSeedAddresses());
  const [search, setSearch] = useState('');
  const [sortKey, setSortKey] = useState<SortKey>('name');
  const [sortDir, setSortDir] = useState<SortDir>('asc');
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<AddressRecord | null>(null);
    ...
    return items
      .filter((item) => {
        ...
        return (
          ...
          formatAddressType(item.addressType).toLowerCase().includes(query)
        );
      })
      .sort((a, b) => {
        const dir = sortDir === 'asc' ? 1 : -1;
        if (sortKey === 'name') return a.name.localeCompare(b.name) * dir;
        if (sortKey === 'addressType') {
          return formatAddressType(a.addressType).localeCompare(formatAddressType(b.addressType)) * dir;
        }
        if (sortKey === 'city') return a.city.localeCompare(b.city) * dir;
        if (sortKey === 'zipCode') return a.zipCode.localeCompare(b.zipCode) * dir;
  ...
  const openEdit = (item: AddressRecord) => {
    setEditingItem(item);
    setDialogOpen(true);
  };

  const handleSave = (saved: AddressRecord) => {
    setItems((prev) => {
    ...
    >
        ...
        >
          <nav aria-label="Breadcrumb" style={{ marginBottom: '20px' }}>
            ...
            >
              ...
              <li style={{ color: P2P_BRAND.primaryStrong, fontWeight: 600 }}>Address Setup</li>
            </ol>
          </nav>
          ...
          >
            <div>
              ...
              >
                Manage company addresses used for delivery, billing, and department/location setup.
              </p>
            </div>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: '14px', fontWeight: 600, color: '#101828' }}>All addresses</span>
                <span
                  style={{
                    padding: '2px 9px',
                    background: '#F2F4F7',
            ...
            {selectedIds.size > 0 && (
              ...
              >
                ...
                <button
                  ...
                  aria-label={`Delete ${selectedIds.size} selected addresses`}
                  style={{
                    display: 'inline-flex',
                    alignItems: 'center',
              ...
              >
                <thead>
                  <tr style={{ background: '#F9FAFB', borderBottom: '1px solid #E4E7EC' }}>
                    ...
                    <SortableHeader
                      label="Address type"
                      active={sortKey === 'addressType'}
                      onClick={() => toggleSort('addressType')}
                    />
                    <SortableHeader label="City" active={sortKey === 'city'} onClick={() => toggleSort('city')} />
                    <SortableHeader
                      label="Zip code"
                      active={sortKey === 'zipCode'}
                  ...
                  {filtered.length === 0 ? (
                    ...
                    paginatedItems.map((item) => (
                      ...
                      >
                        ...
                        <td style={{ ...tdStyle, fontSize: '13px', color: '#334155' }}>{item.city}</td>
                        <td style={{ ...tdStyle, fontSize: '13px', color: '#334155' }}>{item.zipCode}</td>
                        <td style={{ ...tdStyle, fontSize: '13px', color: '#334155' }}>{item.phone}</td>
      ...
      <AlertDialog open={Boolean(deletePrompt)} onOpenChange={(open) => !open && setDeletePrompt(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {deletePrompt?.type === 'bulk' ? 'Delete selected addresses?' : 'Delete address?'}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {deletePrompt?.type === 'bulk'
                ? `${deletePrompt.count} addresses will be permanently removed.`
