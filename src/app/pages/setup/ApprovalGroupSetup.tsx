...
import { ApprovalGroupFormDialog } from '../../components/setup/ApprovalGroupFormDialog';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
type SortKey = 'name' | 'users' | 'active';
type SortDir = 'asc' | 'desc';
type DeleteMode = { type: 'single'; id: string; name: string } | { type: 'bulk'; count: number };

export function ApprovalGroupSetup() {
  const [items, setItems] = useState<ApprovalGroup[]>(() => createSeedApprovalGroups());
  const [search, setSearch] = useState('');
  const [sortKey, setSortKey] = useState<SortKey>('name');
  const [sortDir, setSortDir] = useState<SortDir>('asc');
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<ApprovalGroup | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return items
      .filter((item) => {
        ...
        const users = formatUserNames(item.userIds).toLowerCase();
        return item.name.toLowerCase().includes(query) || users.includes(query);
      })
      .sort((a, b) => {
        const dir = sortDir === 'asc' ? 1 : -1;
        if (sortKey === 'name') return a.name.localeCompare(b.name) * dir;
        if (sortKey === 'active') return (Number(a.active) - Number(b.active)) * dir;
        return formatUserNames(a.userIds).localeCompare(formatUserNames(b.userIds)) * dir;
      });
  ...
  const openEdit = (item: ApprovalGroup) => {
    setEditingItem(item);
    setDialogOpen(true);
  };

  const handleSave = (saved: ApprovalGroup) => {
    setItems((prev) => {
    ...
    >
        ...
        >
          <nav aria-label="Breadcrumb" style={{ marginBottom: '20px' }}>
            ...
            >
              ...
              <li style={{ color: P2P_BRAND.primaryStrong, fontWeight: 600 }}>Approval Group</li>
            </ol>
          </nav>
          ...
          >
            <div>
              ...
              >
                Define groups of users who can approve purchase requests, invoices, and other
                transactions.
              </p>
            </div>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: '14px', fontWeight: 600, color: '#101828' }}>All groups</span>
                <span
                  style={{
                    padding: '2px 9px',
                    background: '#F2F4F7',
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  aria-label="Search approval groups"
                  style={{
                    height: '36px',
                    width: '240px',
            ...
            {selectedIds.size > 0 && (
              ...
              >
                ...
                <button
                  ...
                  aria-label={`Delete ${selectedIds.size} selected groups`}
                  style={{
                    display: 'inline-flex',
                    alignItems: 'center',
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr>
                      ...
                      >
                        {items.length === 0
                          ? 'No approval groups yet. Add your first group to get started.'
                    ...
                    paginatedItems.map((item) => (
                      ...
                      >
                        ...
                        <td style={{ ...tdStyle, color: '#334155', fontSize: '13px', lineHeight: 1.45 }}>
                          {formatUserNames(item.userIds) || '—'}
                        </td>
      ...
      <AlertDialog open={Boolean(deletePrompt)} onOpenChange={(open) => !open && setDeletePrompt(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {deletePrompt?.type === 'bulk' ? 'Delete selected groups?' : 'Delete approval group?'}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {deletePrompt?.type === 'bulk'
                ? `${deletePrompt.count} approval groups will be permanently removed.`
