...
import { FilterProfileFormDialog } from '../../components/setup/FilterProfileFormDialog';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import {
  createSeedFilterProfiles,
  type FilterProfile,
} from '../../data/filterProfileSetup';
import { P2P_BRAND } from '../../tokens/brand';
import { UI_FONT_STACK as F } from '../../tokens/typography';

type SortKey = 'name' | 'project' | 'active';
type SortDir = 'asc' | 'desc';
type DeleteMode = { type: 'single'; id: string; name: string } | { type: 'bulk'; count: number };

export function FilterProfileSetup() {
  const [items, setItems] = useState<FilterProfile[]>(() => createSeedFilterProfiles());
  const [search, setSearch] = useState('');
  const [sortKey, setSortKey] = useState<SortKey>('name');
  const [sortDir, setSortDir] = useState<SortDir>('asc');
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<FilterProfile | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return items
      ...
      .sort((a, b) => {
        ...
        if (sortKey === 'project') return (Number(a.projectEnabled) - Number(b.projectEnabled)) * dir;
  ...
  const openEdit = (item: FilterProfile) => {
    setEditingItem(item);
    setDialogOpen(true);
  };

  const handleSave = (saved: FilterProfile) => {
    setItems((prev) => {
    ...
    >
        ...
        >
          <nav aria-label="Breadcrumb" style={{ marginBottom: '20px' }}>
            ...
            >
              ...
              <li style={{ color: P2P_BRAND.primaryStrong, fontWeight: 600 }}>Filter Profiles</li>
            </ol>
          </nav>
          ...
          >
            <div>
              ...
              >
                Create filter profiles to limit what users see by department, location, account, or
                project.
              </p>
            </div>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: '14px', fontWeight: 600, color: '#101828' }}>All profiles</span>
                <span
                  style={{
                    padding: '2px 9px',
                    background: '#F2F4F7',
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  aria-label="Search filter profiles"
                  style={{
                    height: '36px',
                    width: '240px',
            ...
            {selectedIds.size > 0 && (
              ...
              >
                ...
                <button
                  ...
                  aria-label={`Delete ${selectedIds.size} selected profiles`}
                  style={{
                    display: 'inline-flex',
                    alignItems: 'center',
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr>
                      ...
                      >
                        {items.length === 0
                          ? 'No filter profiles yet. Add your first profile to get started.'
                    ...
                    paginatedItems.map((item) => (
                      ...
                      >
                        ...
                        <td style={tdStyle}>
                          <ProjectBadge enabled={item.projectEnabled} />
                        </td>
      ...
      <AlertDialog open={Boolean(deletePrompt)} onOpenChange={(open) => !open && setDeletePrompt(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {deletePrompt?.type === 'bulk' ? 'Delete selected profiles?' : 'Delete filter profile?'}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {deletePrompt?.type === 'bulk'
                ? `${deletePrompt.count} filter profiles will be permanently removed.`
...
function ProjectBadge({ enabled }: { enabled: boolean }) {
  return (
    <span
      style={{
        fontSize: '13px',
